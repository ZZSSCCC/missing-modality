...
class BreastDataset_clinical(torch.utils.data.Dataset):
    """Pytorch dataset api for loading patches and preprocessed clinical data of breast."""

    def __init__(self, json_path, data_dir_path='./dataset', clinical_data_path=None, mode='train'):
        ...
        for idx in range(len(self.json_data)):
            if self.json_data[idx]['id'] not in self.id_labels.keys():
                self.id_labels[self.json_data[idx]['id']]=self.json_data[idx]['label']
    ...
    def __getitem__(self, index):
        ...
        label = int(self.id_labels[patient_id])

        data={}
        data["clinical_data"] = torch.from_numpy(self.clinical_data_df.loc[int(patient_id)].to_numpy())
        data["label"] = label
...
class BreastDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, json_path, data_dir_path='./dataset', clinical_data_path=None, is_preloading=True, mode='train'):
        self.data_dir_path = data_dir_path
    ...
    def __getitem__(self, index):
        ...
        if self.clinical_data_df is not None:
            ...
            if self.mode == 'test':
                ...
                r = random.random()
                if r <= 0.8:
                    data["clinical_data"] = np.array([-1, -1, -1, -1, -1])
                    data["miss"] = 1
                else:
                    data["miss"] = 0
    ...
    def generate_data_G(self):
        age = np.random.normal(loc=0.0, scale=1.0)
        tumor_size = np.random.normal(loc=0.0, scale=1.0)
        is_er = random.choice([0, 1])
        is_pr = random.choice([0, 1])
        is_her2 = random.choice([0, 1])
        return np.array([age, tumor_size, is_er, is_pr, is_her2])

    def generate_data_S(self):
        return np.array([0.003188, -0.02074, 0, 0, 1])
